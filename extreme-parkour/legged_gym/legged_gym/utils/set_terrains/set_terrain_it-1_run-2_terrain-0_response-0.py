...
def set_terrain(length, width, field_resolution, difficulty):
    """Challenging a quadruped with staggered and sloped platforms, testing agility and balance."""
    ...
    # Platform dimensions
    platform_length = 0.6 + 0.3 * difficulty  # Make platforms longer with difficulty
    platform_length = m_to_idx(platform_length)
    platform_width = 0.6 + 0.2 * difficulty  # Make platforms wider with difficulty
    platform_width = m_to_idx(platform_width)
    platform_height_min, platform_height_max = 0.1 + 0.2 * difficulty, 0.3 + 0.4 * difficulty
    gap_length = 0.4 + 0.3 * difficulty  # Increase gap length with difficulty
    ...
    for i in range(1, 6):  # Set up 5 mixed ramps and platforms
        dx = np.random.randint(dx_min, dx_max)
        dy = np.random.randint(dy_min, dy_max)
